...
# Method to ensemble text and annotationss
def ensemble_corpus_annotations(text_path:str, annot_path:str):
	# Path
	FILEPATH = os.getcwd()
	# Load title and abstract file
	json_list = json.load(open(os.path.join(FILEPATH, text_path)))["articles"]
	df_text = pd.DataFrame(json_list)
	# Load entities file
	json_list = json.load(open(os.path.join(FILEPATH, annot_path)))["articles"]
	df_entities = pd.DataFrame(json_list)
	# Ensemble all entities in one list per text
	df_entities["tag"] = df_entities["diseases"]
	for i, row in tqdm(df_entities.iterrows(), total=df_entities.shape[0]):
		dis_list = [(dictionary["span"], "disease", dictionary["start"], dictionary["end"]) for dictionary in row["diseases"]]
		med_list = [(dictionary["span"], "medication", dictionary["start"], dictionary["end"]) for dictionary in row["medications"]]
		proc_list = [(dictionary["span"], "procedure", dictionary["start"], dictionary["end"]) for dictionary in row["procedures"]]
		sym_list = [(dictionary["span"], "symptom", dictionary["start"], dictionary["end"]) for dictionary in row["symptoms"]]
		df_entities.loc[i,'tag'] = dis_list + med_list + proc_list + sym_list
	# Get final object
	final_df = pd.merge(left=df_text[["id", "title", "abstractText"]], right=df_entities[["id", "tag"]], how = "right", on = "id")
	# Write data in spaCy 3.x format
	annotations = []
	for i, row in tqdm(final_df.iterrows(), total=final_df.shape[0]):
		annotations.append(
			{
				'entities':[(int(dictionary[2]), int(dictionary[3]), dictionary[1]) for dictionary in row['tag']]
				,
				'text': row['abstractText']
			}
		)
	return annotations

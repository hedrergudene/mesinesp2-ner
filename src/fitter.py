...
# Fitter method
class NER_Fitter(TorchFitterBase):
        ...
        for e in range(n_epochs):
            ...
            if validation_callbacks is not None:
                if not isinstance(validation_callbacks, list):
                    validation_callbacks = [validation_callbacks]
                for c in validation_callbacks:
                    c(history, step=(self.epoch+1)*len(train_loader))
    ...
    # Epoch
    def train_one_epoch(self, train_loader, step_callbacks=None, verbose_steps=0):
        """
        Run one epoch on the train dataset
        Parameters
        ----------
                ...
                if step % verbose_steps == 0:
                    ...
                    if (step_callbacks is not None) & (step>0):
                        ...
                        for c in step_callbacks:
                            c({'train_ner_loss':summary_ner_loss.avg,
                               'lr':self.optimizer.param_groups[0]['lr']}, step=self.epoch*len(train_loader)+step)
            # Unpack batch of data
            x, y, w = self.unpack(data)
